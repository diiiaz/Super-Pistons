...
public class SuperPistonHeadBlock extends FacingBlock {
    public static final EnumProperty<SuperPistonType> TYPE = ModProperties.MOD_PISTON_TYPE;
...
//    protected static final float field_31380 = 10.0f;
    ...
    private static final VoxelShape[] SHORT_HEAD_SHAPES = SuperPistonHeadBlock.getHeadShapes(true);
    private static final VoxelShape[] HEAD_SHAPES = SuperPistonHeadBlock.getHeadShapes(false);

    private static VoxelShape[] getHeadShapes(boolean shortHead) {
        return (VoxelShape[]) Arrays.stream(Direction.values()).map(direction -> SuperPistonHeadBlock.getHeadShape(direction, shortHead)).toArray(VoxelShape[]::new);
    }
    ...
    public SuperPistonHeadBlock(AbstractBlock.Settings settings) {
        super(settings);
        this.setDefaultState((BlockState)((BlockState)((BlockState)((BlockState)this.stateManager.getDefaultState()).with(FACING, Direction.NORTH)).with(TYPE, SuperPistonType.DEFAULT)).with(SHORT, false));
    }
    ...
    private boolean isAttached(BlockState headState, BlockState pistonState) {
        Block block = headState.get(TYPE) == SuperPistonType.DEFAULT ? ModBlocks.SUPER_PISTON : ModBlocks.SUPER_STICKY_PISTON;
        return pistonState.isOf(block) && pistonState.get(SuperPistonBlock.EXTENDED) && pistonState.get(FACING) == headState.get(FACING);
    }
    ...
    public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        ...
        return this.isAttached(state, blockState) || blockState.isOf(ModBlocks.MOVING_SUPER_PISTON) && blockState.get(FACING) == state.get(FACING);
    }
    ...
    public ItemStack getPickStack(BlockView world, BlockPos pos, BlockState state) {
        return new ItemStack(state.get(TYPE) == SuperPistonType.DEFAULT ? ModBlocks.SUPER_PISTON : ModBlocks.SUPER_STICKY_PISTON);
    }
