...
public class SuperPistonHandler {
    public static final int MAX_MOVABLE_BLOCKS = 3;
    ...
    public SuperPistonHandler(World world, BlockPos pos, Direction dir, boolean extended) {
        this.world = world;
        this.posFrom = pos;
    ...
    public boolean calculatePush() {
        ...
        if (!SuperPistonBlock.isMovable(blockState, this.world, this.posTo, this.motionDirection, false, this.pistonDirection)) {
            if (this.extended && blockState.getPistonBehavior() == PistonBehavior.DESTROY) {
        ...
        for (BlockPos blockPos : this.movedBlocks) {
            if (!SuperPistonHandler.isBlockSticky(this.world.getBlockState(blockPos)) || this.tryMoveAdjacentBlock(blockPos))
                continue;
            return false;
        }
        return true;
    ...
    private static boolean isAdjacentBlockStuck(BlockState state, BlockState adjacentState) {
        ...
        return SuperPistonHandler.isBlockSticky(state) || SuperPistonHandler.isBlockSticky(adjacentState);
    }
    ...
    private boolean tryMove(BlockPos pos, Direction dir) {
        ...
        if (!SuperPistonBlock.isMovable(blockState, this.world, pos, this.motionDirection, false, dir)) {
            return true;
        }
        ...
        while (SuperPistonHandler.isBlockSticky(blockState)) {
            BlockPos blockPos = pos.offset(this.motionDirection.getOpposite(), i);
            BlockState blockState2 = blockState;
            blockState = this.world.getBlockState(blockPos);
            if (blockState.isAir() || !SuperPistonHandler.isAdjacentBlockStuck(blockState2, blockState) || !SuperPistonBlock.isMovable(blockState, this.world, blockPos, this.motionDirection, false, this.motionDirection.getOpposite()) || blockPos.equals(this.posFrom)) break;
        ...
        while (true) {
            ...
            if ((l = this.movedBlocks.indexOf(blockPos2 = pos.offset(this.motionDirection, k))) > -1) {
                ...
                for (int m = 0; m <= l + j; ++m) {
                    ...
                    if (!SuperPistonHandler.isBlockSticky(this.world.getBlockState(blockPos3)) || this.tryMoveAdjacentBlock(blockPos3)) continue;
                    return false;
                }
                return true;
            }
            ...
            if (!SuperPistonBlock.isMovable(blockState, this.world, blockPos2, this.motionDirection, true, this.motionDirection) || blockPos2.equals(this.posFrom)) {
                return false;
            }
    ...
    private boolean tryMoveAdjacentBlock(BlockPos pos) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (direction.getAxis() == this.motionDirection.getAxis() || !SuperPistonHandler.isAdjacentBlockStuck(blockState2 = this.world.getBlockState(blockPos = pos.offset(direction)), blockState) || this.tryMove(blockPos, direction)) continue;
            return false;
        }
        return true;
    }
